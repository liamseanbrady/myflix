...
feature "User interacts with advanced search", :js, :elasticsearch do
  background do
  ...
  scenario "by searching with title" do
    ...
    expect(page).not_to have_content('Star Trek')
  end
  ...
  scenario "user filters search results with average rating", driver: :selenium do
    ...
    expect(page).not_to have_content "Star Wars: Episode 1"
    expect(page).not_to have_content "Star Wars: Episode 2"
  end
